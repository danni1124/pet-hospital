...
export default {
  ...
  methods: {
    ...
    fetchProducts() {
      ...
      this.error = '';
        ...
        .then((res) => {
          ...
          console.log("全部数据",this.products);
        })
        .catch((error) => {
    ...
    applyCoupon() {
      ...
      axios.post(`${API_BASE_URL}/coupon`, { code: this.couponCode })
        .then((res) => {
          if (res.data.valid) {
            this.discountAmount = res.data.discountAmount;
            this.showNotification = true;
            this.notificationMessage = '优惠码已应用！';
          } else {
            this.showNotification = true;
        ...
        .catch((error) => {
          ...
          this.showNotification = true;
          this.notificationMessage = '应用优惠券失败，请重试';
        })
        .finally(() => {
          setTimeout(() => {
            this.showNotification = false;
...
submitPayment() {
  ...
  const order = {
    orderNo: orderNo,
    userId: 1, // 假设用户ID为1
    totalAmount: this.cartTotal - this.discountAmount, // 使用 totalAmount 而不是 total
    status: 1, // 假设1表示待支付状态
